...
heruntergeladen haben.  Die Koordinaten dafür sind in
Abschnitt~\ref{sec:racket} auf Seite~\pageref{sec:racket}.
...
dieses Buch noch auswählen.  Dazu wähle den Menüpunkt
...
\end{lstlisting}
%
Ein solcher Name in einem Programm heißt \textit{Variable\index{Variable}}.
...
\end{lstlisting}
%
Du kannst also auch diese Namen in die REPL eingeben und bekommst
...
\lstinline{string-length}\index{string-length@\texttt{string-length}}
...
\lstinline{string->number}\index{string->number@\texttt{string->number}}
...
\end{lstlisting}
%
Das \lstinline{lambda}\index{lambda@\texttt{lambda}} ist eine Art Zauberwort, es sagt soviel wie: "<Für
...
\begin{feature}{Signaturen}{scheme:signature}
  ...
  \smallskip
  Die Signatur \lstinline{string}\index{string@\texttt{string}}
...
\lstinline{check-expect}\index{check-expect@\texttt{check-expect}} (siehe
...
%
\begin{lstlisting}
(check-expect (square 40 "solid" "red") |\includegraphics[height=24pt]{elemente/square})|
\end{lstlisting}
...
\end{lstlisting}
  ...
  genannt \textit{Ellipse}\index{Ellipse}) als Erinnerung daran, dass
...
\begin{aufgabe} Die \textit{Internetzeit} ist ein alternatives Konzept
  ...
  Nachkommastellen, außer, wenn beide Nachkommastellen 0 sind: In
  diesem Fall gibt es nur eine Nachkommastelle.

  Notiert wird die Zeit durch ein @-Symbol vor dem Wert.
  \texttt{@0.0} ist gleichbedeutend mit 0 Uhr mitteleuropäischer Zeit.
  ...
  Funktion rundet ab, die zweite rundet auf oder ab, je nachdem ob die
  letzte Ziffer unterhalb oder oberhalb von 5 lieft.
\end{aufgabe}
...
\begin{aufgabe}  (etwas schwieriger, mit Physik)
