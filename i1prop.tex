...
\end{alltt}
%
Wenn Du diese Eigenschaft neben die Kommutativität für \texttt{+}
...
\end{alltt}
%
Hier muss die eingebaute Funktion \texttt{boolean=?} verwendet werden,
...
\end{alltt}
%
Denk an das \texttt{signature}, das immer notwendig ist, wenn
...
Argument zu \texttt{op} muss eine Funktion sein~-- \texttt{and} und
...
der Rückgabewert von \texttt{op} auf die gleiche Signatur passt wie die
...
Die Funktion \texttt{=} passt nicht in das Schema der Eigenschaften
...
\end{alltt}
%
Die Symmetrie bedeutet für \texttt{=}, dass aus \texttt{(= a b)
...
\end{alltt}
%
suggeriert allerdings, dass die Signatur von \texttt{lis-1},
...
Generator">: Das liegt daran, dass die Signaturvariable \texttt{\%a}
...
\texttt{number-list=?} interessante Eigenschaften: Wie \texttt{=} muss
...
Eigenschaft von \texttt{invert} ist, dass zweimaliges Umdrehen wieder
...
\end{alltt}
%
Wie lässt sich die Liste \texttt{(invert (concatenate lis-1 lis-2))}
noch beschreiben?
...
\texttt{rational}.  Der fertige Test muss also so aussehen:
%
\begin{alltt}
(check-property
...
Funktionen an, dass sie Wert desselben \texttt{lambda}-Ausdrucks sind.
...
Damit ein \texttt{check-property}-Test die Äquivalenz testen kann, muss
...
\end{alltt}
%
(Diese Eigenschaft lässt sich nicht sinnvoll mit \texttt{for-all}
...
% Das gleiche Spiel lässt sich auch am Beispiel der Funktion \texttt{f}
...
\end{alltt}
%
Wie bei \texttt{factorial} muss zwischen $k=0$ und $k>0$ unterschieden
...
\texttt{acc} ist.  Um die Annahme zu beweisen, dass dies die Invariante
...
\begin{aufgabe}  Schreibe Abstraktionen analog zu \texttt{commutativity} und
...
\begin{aufgabe}
  Versuche, die Eigenschafts-Tests für \texttt{number-list=?}
...
\begin{aufgabe}
  Formuliere Eigenschaften von \texttt{filter} und \texttt{map}
  im Zusammenhang mit \texttt{concatenate} und teste diese.
\end{aufgabe}
...
\begin{aufgabe}
  Schreibe einen \texttt{check-property}-Test für folgende Eigenschaft:
  \begin{center}
...
\begin{aufgabe}
  Formuliere sinnvolle Eigenschaften von \texttt{compose} und
  \texttt{repeat} von Seite~\ref{page:repeat} und überprüfe
...
\begin{aufgabe}
Beweise, dass auch die folgende Funktion \texttt{square}
