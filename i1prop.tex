...
Hier ist eins: Wir schreiben eine Funktion, welche die Mitglieder
...
Da es für \lstline{sort-band} für eine gegebene Eingabe mehr als ein
...
\lstlisting{sort-band} immer sortierte Listen produziert:
%
\begin{lstlisting}
(check-property
...
\end{lstlisting}
%
Diese Eigenschaft bringt auf den Punkt, was \lstlisting{sort-band}
...
Gefühl, dass wir für den Test von \lstlisting{sort-band} eine weitere
selbstgeschriebene Funktion benutzt haben, die fast ebenso kompliziert
ist wie \lstlisting{sort-band} selbst.  Was, wenn wir in
\lstlisting{band-sorted?} einen Fehler gemacht haben, und zwar so,
dass der Eigenschafts-Testfall dann einen Fehler in
\lstlisting{sort-band} nicht mehr findet.  Das ist natürlich
...
ist, eine Funktion wie \lstlisting{band-sorted?} zu schreiben, die nur
