...
Oder\index{inklusives Oder}: \texttt{(or $A$ $B$)} bedeutet "<$A$ oder
$B$ oder beides">, im Gegensatz zum \textit{exklusiven}
Oder\index{exklusives Oder} "<entweder $A$ oder
...
\index{Abdeckung}\index{Testabdeckung}

\begin{figure}[tb]
  \centering
  \includegraphics[width=0.7\textwidth]{i1verzw/coverage}
  \caption{Anzeige von Testabdeckung in DrRacket}
  \label{fig:coverage}
\end{figure}

Vielleicht ist Dir bei der \texttt{say-number}-Funktion aus dem
vorigen Abschnitt aufgefallen, dass das Programm unmittelbar, nachdem
es gelaufen hat, etwas merkwürdig aussieht: Zwei Zeilen sind farbig
hinterlegt, wie in Abbildung~\ref{fig:coverage} zu sehen.  Das liegt
daran, dass diese beiden Zeilen nie ausgewertet wurden, weil die
Tests nur die ersten beiden Fälle 0 und 1 übeprüfen.  Theoretisch
...
Die farbige Hinterlegung zeigt die \textit{Testabdeckung} oder kurz
\textit{Abdeckung} an: Ein Ausdruck im Programm, der durch die Tests
ausgewertet wird, heißt \textit{abgedeckt}.  Die farbig hinterlegten
Ausdrücke sind also nicht abgedeckt.  Der letzte Satz von
Konstruktionsanleitun~\ref{ka:tests} auf Seite~\pageref{ka:tests}
zielt gerade darauf: Eine Funktion sollte durch ihre Tests vollständig
abgedeckt werden.  Die Abdeckung ist also eine Minimalanforderung an
unsere Tests.
