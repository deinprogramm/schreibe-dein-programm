...
$\bot$\index{*@$\bot$} mit den Wahrheitswerten W beziehungsweise F.  Die
...
das heißt die \emph{Reihenfolge\index{Reihenfolge}} der Elemente ist unerheblich (beziehungsweise es gibt
...
Mengen sind so definiert, dass sie jedes Element "<nur einmal enthalten">. 
...
%    Beachte, dass die Gleichheit der Graphen auch die Gleichheit von
...
Eine Abbildung (auch: Funktion) $A \stackrel{f}{\longrightarrow} B$ heißt
    \begin{itemize}
    \item \emph{surjektiv} $\diff$ für alle $b\in B$ gibt es ein $a\in A$, so dass $f(a)=b$,\index{surjektiv}
