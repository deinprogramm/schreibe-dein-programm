...
\section{Die Sprache der SECD-Maschine}
...
bei den "<normalen"> Applikationen keine Applikationen von Primitiven
erwischen:
%
\begin{lstlisting}
...
allernötigsten primitiven Operationen:
%
\begin{lstlisting}
...
\lstinline{application}, nur dass hier nur Primitive erwünscht sind:
%
\begin{lstlisting}
...
halber beschränken wir uns auf boolesche Werte und Zahlen:
%
\begin{lstlisting}
...
(check-expect (term->machine-code 
              ...
              (list (make-abs 'x (list 'x 'x (make-ap)))
                    (make-abs 'x (list 'x 'x (make-ap)))
                    (make-ap)))
\end{lstlisting}
%
...
Hier ist ein Beispiel für eine Umgebung:
%
\begin{lstlisting}
(define env1 (list (make-binding 'slash 57) (make-binding 'axl 60)))
\end{lstlisting}
%
...
(check-expect (lookup-environment env1 'axl) 60)
(check-expect (lookup-environment env1 'slash) 57)
\end{lstlisting}
%
...
Basiswerte wurden bereits in Abschnitt~\ref{sec:secd-datenanalyse} auf
Seite~\pageref{sec:secd-datenanalyse} definiert; es fehlen noch
Closures, also Tupel $V\times C\times E$ sind.  Hier sind die
