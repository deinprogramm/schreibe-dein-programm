...
einfacher ist eine mit gar keinen Elementen~-- die \textit{leere
  Liste} an\index{leere Liste}\index{Liste!leer}.  Natürlich werden
...
\end{lstlisting}
%
Wir müssen dann immer da, wo wir vorher \lstinline{cons-list} stand,
\lstinline{(cons-list |\ldots|)} schreiben und eine Signatur für die
...
ausmerzen: \lstinline{list-of-numbers} ist ja jetzt nicht mehr
definiert, steht aber noch in den Signatur-Deklarationen von
\lstinline{list-sum} und \lstinline{list-product}.  Wir können
entweder dort \lstinline{list-of-numbers} durch \lstinline{(list-of
  number)} ersetzen oder einfach eine Signatur-Definition
dazuschreiben:
%
\begin{lstlisting}
...
und \lstinline{list-product} stehen.

FIXME: Erweiterte Record-Form
