...
\texttt{define-record-functions} mit Prädikaten.

Aus diesem Beispiel ergibt sich eine Konstruktionsanleitung für
gemischte Daten.  Zunächst zur Daten- und Signaturdefinition:
%
\begin{itemize}
\item Gemischte Daten liegen vor, wenn Du eine Datensorte durch eine
  Formulierung der Form "<ein $X$ ist eins der folgenden"> beschreiben
  kannst: Diese Formulierung ist die Datendefinition.
\item Stelle fest, wieviele unterschiedliche Fälle die Sorte
  für die gemischten Daten hat.
\item Schreibe eine Signaturdefinition der folgenden Form unter
  die Datendefinition:
\begin{alltt}
(define \(S\)
  (signature
...
\item Vervollständige die Zweige, indem Du eine Datenanalyse für
  ...
  oder Konstruktionsanleitungen benutzt.
